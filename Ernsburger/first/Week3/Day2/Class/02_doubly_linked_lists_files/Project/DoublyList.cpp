...
{
	Node *newNode = new Node(newData, NULL, NULL);	//Create a pointer newNode to point to a dynamic node
...
void DoublyList::deleteNode(int deleteData) 
{
    ...
    else  
    {
		...
		{
			current = first = first->getNextLink();	//make pointer first point to the next node
